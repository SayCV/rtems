...
at91_usart_t *usart_get_base(int minor);
...
at91_usart_t *usart_get_base(int minor)
{
  ...
  port = (at91_usart_t *) console_entry->ulCtrlPort1;
...
static int usart_first_open(int major, int minor, void *arg)
{
  ...
  usart->cr = (AT91_US_RXEN | AT91_US_TXEN);
  return 0;
}
...
static int usart_set_attributes(int minor, const struct termios *t)
{
  ...
  mode = usart->mr & ~(AT91_US_USMODE_NORMAL | AT91_US_USCLKS | AT91_US_CHRL
			| AT91_US_PAR | AT91_US_NBSTOP);
  ...
  /* Stop bits */
  if (t->c_cflag & CSTOPB){
	mode |= AT91_US_NBSTOP_2; /* 2 stop bits */
  } else
    mode |= AT91_US_NBSTOP_1;     /* 1 stop bits */
  ...
  brgr = (get_mck_clk_rate() / 16) / baud;

	if (brgr > 65535){    /* BRGR is 16-bit, so switch to slower clock */
		brgr /= 8;
		mode |= AT91_US_USCLKS_MCK_DIV8;
	}
