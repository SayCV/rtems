...
static void dbgu_init(int minor)
{
    ...
    dbgu->mr = AT91_US_PAR_NONE | AT91_US_USMODE_NORMAL;

    /* Set the baud rate */
    dbgu->brgr = (get_mck_clk_rate() / 16) / BSP_get_baud();

    /* Enable the DBGU */
    dbgu->cr = (AT91_US_TXEN | AT91_US_RXEN);
}
